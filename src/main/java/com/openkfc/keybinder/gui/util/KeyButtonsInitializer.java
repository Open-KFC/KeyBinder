...
public class KeyButtonsInitializer {
    public final int x, y, width, height;

    public KeyButtonsInitializer(int left, int right, int centerY) {
        this(left, right, centerY, -1);
    }

    public KeyButtonsInitializer(int left, int right, int centerY, int maxHeight) {
        heightLimit:
        {
            if (maxHeight == -1)
                break heightLimit;
    ...
    public void applyTo(GuiScreen screenIn, List<? super KeyButton> outList) {
        int keyScale, keyGap, around; //普通按键的大小，普通按键间的距离，四周的间距
        int[] yLn = new int[6]; //每行的y值
        {
            keyGap = height / 70;
            around = height / 35;
        ...
        int escFuncGap, funcGap; //esc与f区的距离；f区分为四组，每组三个键，每组间的距离
        int wRBackspace, wLTab, wRBackSlash, wLCaps, wREnter, wLShift, wRShift, wLCtrl, wSpace, wRCtrl; //各个特殊键的宽度
        {
